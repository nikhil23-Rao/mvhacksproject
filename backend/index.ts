...
// Setup gemini api
const configuration = new GoogleGenerativeAI("AIzaSyAbTnOKMAaVuRsxZRi_mq0M4HsZmQwO6hM" as string);
const ai = new GoogleGenAI({ apiKey: "AIzaSyAbTnOKMAaVuRsxZRi_mq0M4HsZmQwO6hM" });
const model = configuration.getGenerativeModel({ model: "gemini-2.5-pro" });
...
export const textToAudio = async (req: Request, res: Response) => {
  try {
    ...
    const response = await ai.models.generateContent({
      // 1. Use the specific TTS model
      model: "gemini-2.5-flash-preview-tts",
      // 2. Just pass the raw text you want spoken
      contents: text,
      config: {
    ...
    if (!audioPart) {
      return res.status(500).json({ message: "Failed to generate audio from Gemini." });
    }
...
export const tutor = async (req: Request, res: Response) => {
  try {
    ...
    const responseText = response;
    console.log((responseText as any).candidates[0].content.parts[0].text);
...
app.post("/text-to-audio", async (req: Request, res: Response) => {
  await textToAudio(req, res);
});
...
app.post("/resourcefinder", async (req: Request, res: Response) => {
  console.log("Received query for resource finder:", req.body.query);
  const { query } = req.body;
