...
// Helper function to wrap raw PCM base64 data in a WAV header
function createWavFromPcm(base64Data, sampleRate = 24000) {
  ...
  // 2. Create the WAV Header (44 bytes long)
  ...
  // 3. Append the raw PCM Data
  new Uint8Array(buffer, 44).set(bytes);

  // 4. Return a playable Blob URL
  const blob = new Blob([buffer], { type: 'audio/wav' });
  return URL.createObjectURL(blob);
}

// Your updated play function
async function playGeneratedAudio(text, messageId, onPlayStart, onPlayEnd) {
  try {
    onPlayStart(messageId);
    const response = await fetch("http://localhost:3001/text-to-audio", {
      method: "POST",
    ...
    if (data.audioBase64) {
      // Convert the raw PCM into a playable WAV file URL
      const audioSrc = createWavFromPcm(data.audioBase64, 24000);
      console.log("Playable Audio URL:", audioSrc);
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                ...
                                <i className="fa fa-volume-up ml-4" style={{color: playingAudioId === m.id ? "orange" : "#fff", cursor:"pointer", fontSize:20, marginTop:5, transition: "0.2s ease-in"}} onClick={() => playGeneratedAudio(m.message, m.id, (id) => setPlayingAudioId(id), () => setPlayingAudioId(null))}></i>
                                </div>
                                <p
                                  style={{
                                    marginLeft: 10,
