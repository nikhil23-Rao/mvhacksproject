      ...
      <NewModal modal={quizModal} setModal={setQuizModal} showClose={true}>
        ...
        >
          <h1
            ...
            style={{ fontSize: "4vw", marginTop: -100 }}
          >
            {problemSetName}
          </h1>
          <p style={{ marginTop: 0 }}>{problemSetDescription}</p>
          <TextField
    ...
    return (
      ...
      >
        ...
        >
          ...
          <FormControl style={{ width: 400, marginTop: 60, marginBottom: 20, color:"#fff", background:"#fff" }}>
                ...
                <div className="mx-auto w-full max-w-5xl px-4 py-24 md:px-6">
                  <div className="mx-auto grid max-w-xs items-start gap-6 lg:max-w-none lg:grid-cols-3">
                    {userProblemSets.map((pset) => {
                      return (
                        <div
                          className="rounded-2xl shadow-lg hover:scale-105 hover:shadow-2xl transition-all duration-300 cursor-pointer p-6 flex flex-col items-center justify-center text-center min-h-[250px] focus-visible:outline-none focus-visible:ring focus-visible:ring-indigo-300"
                          style={{backgroundColor: "#2c4ffe"}}
                          tabIndex={0}
                          onClick={() => router.push(`/ps/${pset.docid}`)}
                        >
                        
                          <div className="text-white/70 text-sm mb-2 uppercase tracking-wide">{pset.chosenClass}</div>
                          <h2 className="text-white text-2xl font-bold mb-4 leading-tight">{pset.problemSetName}</h2>
                          <div className="text-white/50 text-xs mb-6">{pset.type} • {pset.public ? 'Public' : 'Private'}</div>
                          <div className="flex justify-center space-x-2">
                            <button
                              className="inline-flex justify-center whitespace-nowrap rounded-lg bg-red-50 px-3 py-2 text-sm font-medium text-red-500 transition-colors hover:bg-red-100 focus-visible:outline-none focus-visible:ring focus-visible:ring-red-300"
