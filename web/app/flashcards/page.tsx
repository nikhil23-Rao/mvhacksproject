...
const coralImages = [
  "/images/corals/coral1.jpg",
  "/images/corals/coral2.jpg",
  "/images/corals/coral3.jpg",
  "/images/corals/coral4.jpg",
  "/images/corals/coral5.jpg",
];

const coralFunFacts = [
  "Did you know? Coral reefs support over 25% of all marine species, despite covering less than 1% of the ocean floor!",
  "Fun fact: Some corals can live for hundreds of years. The oldest known coral is over 4,000 years old!",
  "Interesting: Corals build reefs by forming limestone skeletons. A single reef can grow just 1-3 cm per year.",
  "Did you know? Coral polyps are actually tiny animals, not plants. They catch food from the water around them!",
  "Fun fact: When corals bleach, they expel the colorful algae living in their tissues, turning white. This is a stress response.",
  "Interesting: Corals have a special relationship with zooxanthellae algae. The algae provide food through photosynthesis!",
  "Did you know? Coral reefs are often called the 'rainforests of the sea' because they're so rich in biodiversity!",
  "Fun fact: Some corals can reproduce by breaking off and floating to a new location to start a new colony!"
];
...
export default function Flashcards() {
  ...
  const [currentCoralImage, setCurrentCoralImage] = useState("");
  const [progressCount, setProgressCount] = useState(0);
  const [coralCheckpoints, setCoralCheckpoints] = useState<number[]>([]);
  const [coralResponses, setCoralResponses] = useState<{[key:number]:string}>({});
  const [showCoralFunFact, setShowCoralFunFact] = useState(false);
  const [currentCoralFunFact, setCurrentCoralFunFact] = useState("");
  ...
  // Set coral checkpoints when a set is opened
  useEffect(() => {
    if (currentSetInView.length > 0) {
      const total = currentSetInView.length;
      const checkpoints = [
        Math.ceil(total / 3),
        Math.ceil((2 * total) / 3),
        total
      ];
      setCoralCheckpoints(checkpoints);
    }
  }, [currentSetInView]);
  ...
  // Show coral modal after each third of the set
  const handleProgressUpdate = () => {
    const totalFlashcards = currentSetInView.length;
    const nextProgress = progressCount + 1;
    // Only show modal if at a checkpoint and not already answered for this checkpoint
    if (
      coralCheckpoints.includes(nextProgress) &&
      !coralResponses[nextProgress]
    ) {
      const randomImage =
        coralImages[Math.floor(Math.random() * coralImages.length)];
      setCurrentCoralImage(randomImage);
      
      // Alternate between question and fun fact
      const shouldShowFunFact = nextProgress % 2 === 0;
      if (shouldShowFunFact) {
        const randomFunFact = coralFunFacts[Math.floor(Math.random() * coralFunFacts.length)];
        setCurrentCoralFunFact(randomFunFact);
        setShowCoralFunFact(true);
      } else {
        setShowCoralFunFact(false);
      }
      
      setShowCoralModal(true);
    } else {
      setProgressCount(nextProgress);
    }
  };

  const handleCoralResponse = (response?: string) => {
    // Save response for this checkpoint
    const checkpoint = progressCount + 1;
    setCoralResponses((prev) => ({ ...prev, [checkpoint]: response || "viewed" }));
    setShowCoralModal(false);
    setShowCoralFunFact(false);
    setProgressCount(checkpoint);
  };
  ...
  return (
    <>
      <div
        style={{
          height: !loadingSets ? "100vh" : "200vh",
          width: "100%",
          backgroundColor:
        ...
        >
          {currentCardInView && (
            <Flipper
              ...
              currentCoralImage={currentCoralImage}
              handleCoralResponse={handleCoralResponse}
              showCoralFunFact={showCoralFunFact}
              currentCoralFunFact={currentCoralFunFact}
              theme={theme}
              color={"lightgray"}
            />
          )}
        ...
        >
          ...
          {userFlashCardSets.length > 0 ? (
            <div className="w-full max-w-5xl mx-auto mt-10">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-10 gap-x-8 justify-center">
                {userFlashCardSets
                  .filter(
                    (s) =>
                      s.cardsetName
                  ...
                  .map((set) => (
                    <SetView
                      ...
                      setCoralResponses={setCoralResponses}
      ...
      {/* Update Flipper or progress tracking logic */}
      {currentCardInView && (
        <Flipper
          cardFlipped={cardFlipped}
          quizletModal={quizletModal}
